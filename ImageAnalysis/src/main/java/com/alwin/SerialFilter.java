...
public class SerialFilter {
    ...
    public SerialFilter(BufferedImage originalImage, String filterType, int filterWidth, int filterHeight, int[] weights) {
    ...
    public BufferedImage filter () throws NullPointerException {
        ...
        for (int x = 0; x < filterImage.getWidth(); x ++) {
            for (int y = 0; y < filterImage.getHeight(); y ++) {
                ...
                filterImage.setRGB(x, y, newPixelValue); // if newPixelValue== -1, there is an error // TODO throw an error?
            }
        }
